...
{
    public class WhenBuildingPostSendReminderEmailRequest
    {
        [Test, AutoData]
        public void Then_The_Url_Is_Correctly_Constructed(Guid id, Guid courseDemandId)
        {
            //Act
            var actual = new PostSendReminderEmailRequest(id, courseDemandId);
            
            //Assert
            actual.PostUrl.Should().Be($"demand/{courseDemandId}/send-reminder-email/{id}");
        }
    }
}